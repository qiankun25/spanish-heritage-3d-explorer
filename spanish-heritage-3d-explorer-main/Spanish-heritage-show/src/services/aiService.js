...
class AIService {
  ...
  buildSystemPrompt(guide, monument, language) {
    ...
    let systemPrompt = basePrompts[language]?.[guide] || basePrompts.zh[guide];
    
    // Add monument-specific context
    if (monument) {
      const monumentContext = {
        zh: `当前你正在为游客介绍${monument.name?.zh || monument.name}，位于${monument.location?.city}。这座古迹建于${monument.period}。`,
        en: `Currently you are introducing ${monument.name?.en || monument.name} to visitors, located in ${monument.location?.city}. This monument was built in ${monument.period}.`,
        es: `Actualmente estás presentando ${monument.name?.es || monument.name} a los visitantes, ubicado en ${monument.location?.city}. Este monumento fue construido en ${monument.period}.`
      };
      systemPrompt += ` ${monumentContext[language] || monumentContext.zh}`;
    }
  ...
  async getResponse(prompt, context = {}) {
    ...
    console.log('🤖 AI Service: 开始处理请求', { prompt: prompt.substring(0, 50), guide, language });
    ...
    try {
      ...
      console.log('✅ AI Service: 响应处理完成', { responseLength: response.length });
      return response;
